...
export default defineTool({
  ...
  handler: async ({ text }) => {
    ...
    const { fieldResults, flags, verdict } = analyze(extracted);
    return {
      content: [
        {
          type: "text",
          text: `Verdict: ${verdict}\n\n${JSON.stringify({ extracted, fieldResults, flags }, null, 2)}`,
        },
      ],
      structuredContent: { extracted, fieldResults, flags, verdict },
    };
  },
});
