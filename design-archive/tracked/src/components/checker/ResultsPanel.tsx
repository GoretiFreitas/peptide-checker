    ...
    return (
      <PanelShell>
        <div className="flex flex-col items-center gap-3.5 text-center">
          <div className="flex h-12 w-12 items-center justify-center rounded-2xl bg-white border border-[rgba(200,165,125,0.3)] text-[#B88B60] shadow-xs">
    ...
    return (
      <PanelShell>
        <div className="flex flex-col items-center gap-4 text-center">
          <div className="h-8 w-8 animate-spin rounded-full border-2 border-[rgba(200,165,125,0.3)] border-t-[#B88B60]" />
          <div className="text-[11px] font-semibold tracking-[0.2em] uppercase text-foreground/70">
            Reading certificate…
          </div>
        </div>
    ...
    return (
      <PanelShell>
        <div className="flex flex-col items-center gap-3 text-center">
          <div className="flex h-12 w-12 items-center justify-center rounded-2xl bg-red-50 border border-red-200 text-destructive">
...
function PanelShell({ children }: { children: React.ReactNode }) {
  return (
    <div className="clinical-card flex min-h-[420px] items-center justify-center p-8">
      {children}
    </div>
  );
}
...
function Results({
  ...
  return (
    <div className="clinical-panel space-y-8 p-6 sm:p-8 md:p-10">
      <div className="flex flex-col gap-3 md:flex-row md:items-start md:justify-between border-b border-[rgba(212,175,135,0.2)] pb-6">
        <div>
          <h2 className="text-[11px] font-bold tracking-[0.2em] uppercase text-[#B88B60]">
            Overall outcome
          </h2>
          <div className="mt-2.5">
            <VerdictBadge verdict={verdict} />
          </div>
          <p className="mt-3.5 max-w-[70ch] text-sm leading-relaxed text-foreground font-medium">
            {summarize(result)}
          </p>
      ...
      {missingCritical && (
        <div className="flex gap-3.5 rounded-2xl border border-[--badge-fail-fg]/30 bg-[--badge-fail-fg]/5 p-5">
          <ShieldAlert className="h-5 w-5 shrink-0 text-[--badge-fail-fg]" strokeWidth={1.8} />
          <div>
...
function RegisterBox({ info }: { info: RegisterInfo }) {
  ...
  return (
    <div className="rounded-2xl border border-[rgba(200,165,125,0.3)] bg-white/70 p-5 shadow-xs">
      <div className="flex items-center gap-2 text-[10px] font-bold tracking-[0.2em] uppercase text-[#B88B60]">
      ...
      <a
        href={shareUrl}
        className="mt-3 inline-flex items-center gap-1 text-xs font-semibold text-foreground underline underline-offset-4 hover:text-[#B88B60]"
      >
...
function Row({
  ...
  return (
    <div className="grid grid-cols-1 gap-1 border-b border-[rgba(212,175,135,0.18)] py-3.5 last:border-b-0 md:grid-cols-[220px_1fr_auto] md:items-center md:gap-4">
...
function FieldsList({
  ...
  return (
    <div>
      <h2 className="mb-3 text-[11px] font-bold tracking-[0.2em] uppercase text-[#B88B60]">
        Extracted fields
      </h2>
      <div className="rounded-2xl border border-[rgba(212,175,135,0.25)] bg-white/70 px-5 shadow-xs">
      ...
      {extracted.rawNotes && (
        <details className="mt-4">
          <summary className="cursor-pointer text-[11px] font-semibold tracking-[0.18em] uppercase text-muted-foreground hover:text-foreground">
            Verbatim quotes from certificate
          </summary>
          <pre className="mt-2 whitespace-pre-wrap rounded-xl border border-[rgba(212,175,135,0.25)] bg-white/70 p-4 font-mono text-xs text-muted-foreground">
            {extracted.rawNotes}
          </pre>
        </details>
      )}
...
function FlagsList({ flags }: { flags: Flag[] }) {
  if (flags.length === 0) {
    return (
      <div>
        <h2 className="mb-2.5 text-[11px] font-bold tracking-[0.2em] uppercase text-[#B88B60]">
          Flags
        </h2>
    ...
    <div>
      ...
      <ul className="space-y-3">
        {sorted.map((f, i) => (
          <li
            key={i}
            className="flex gap-3.5 rounded-xl border border-[rgba(212,175,135,0.22)] bg-white/80 p-4 shadow-xs"
          >
            ...
            <div className="flex-1">
              ...
              <div className="text-sm font-medium text-foreground">{f.message}</div>
            </div>
          </li>
        ))}
      </ul>
  ...
  const color =
    level === "high"
      ...
      : level === "medium"
        ? "bg-[--badge-warn-fg]"
        : "bg-[--badge-neutral-fg]";
  return <span className={`mt-1.5 inline-block h-2.5 w-2.5 shrink-0 rounded-full ${color}`} />;
}

function DisclaimerBox() {
  return (
    <div className="rounded-xl border border-[rgba(212,175,135,0.25)] bg-white/60 p-5 text-sm leading-relaxed text-foreground">
  ...
  return (
    <div className="space-y-4 pt-2">
      ...
      <Link
        ...
        className="btn-primary inline-flex items-center rounded-full px-5 py-2.5 text-[11px] font-semibold tracking-[0.18em]"
      >
